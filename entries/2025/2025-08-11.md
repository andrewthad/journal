...
complicated, but it is also relies on COW, which results a compiler inserting
...
we run an operation that merely requires read-only access to mutable data.
...
we have in the function body is that the arrays do not alias one another.
